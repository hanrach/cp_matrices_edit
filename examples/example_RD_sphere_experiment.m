...
for dx = xdx
    ...
    [x2c, y2c, z2c]  = sph2cart(theta - angle, phi, r);
    ...
    Eturn = interp3_matrix(x1d, y1d, z1d, x2c, y2c, z2c, p, band);
    end
    ...
    xs = rng; xrand = rand(size(x));
    ys = rng; yrand = rand(size(y));
    zs = rng; zrand = rand(size(z));

    rng(xs); xrandt = rand(size(x2));
    rng(ys); yrandt = rand(size(y2));
    rng(zs); zrandt = rand(size(z2));
%     [theta_r, phi_r, r_r] = cart2sph(xrand,yrand,zrand);
%     [xrandt, yrandt, zrandt] = sph2cart(theta_r + angle, phi_r, r_r);

    pert = (1/4)*exp(-(10*(z-.1)).^2) + (1/4)*exp(-(10*(x-.1)).^2)+(1/4)*sin(3*(theta)); % +0.4*xrand; %(1/4)*sin(7*(theta)); %+ 0.5*xrand;
    u0 = 1-pert;  v0 = 0.5*pert;
    u = u0;  v = v0;
    
    pert_rotate = (1/4)*exp(-(10*(z-.1)).^2) +(1/4)*exp(-(10*(x2-.1)).^2)+ (1/4)*sin(3*(theta+angle)); % +0.4*xrandt; 
